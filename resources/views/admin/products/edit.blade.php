                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data">
                        @csrf @method('PATCH') <div>
                            ...
                            <input id="name" name="name" type="text" value="{{ old('name', $product->name) }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" required>
                            @error('name') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>

                        <div class="mt-4">
                            <label for="image" class="block font-medium text-sm text-gray-700">Ganti Gambar Produk (Opsional)</label>
                            <input id="image" name="image" type="file" class="block mt-1 w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                            @error('image') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror

                            @if ($product->image)
                                <div class="mt-4">
                                    <span class="block text-sm font-medium text-gray-500 mb-1">Gambar Saat Ini:</span>
                        ...
                        <div class="mt-4">
                            ...
                            <input id="price" name="price" type="number" min="0" value="{{ old('price', $product->price) }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" required>
                            @error('price') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>

                        <div class="mt-4">
                            <label for="description" class="block font-medium text-sm text-gray-700">Deskripsi (Opsional)</label>
                            <textarea id="description" name="description" rows="3" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">{{ old('description', $product->description) }}</textarea>
                            @error('description') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
                        
                        <div class="mt-4">
                            <label for="is_available" class="block font-medium text-sm text-gray-700">Status Ketersediaan</label>
                            <select name="is_available" id="is_available" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" required>
                            ...
                            @error('is_available') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>
